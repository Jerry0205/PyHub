    ...
    try:
        ...
        if existing_entry:
            ...
            if score > entry["score"]:
                ...
                print(
                    f"Score {score} nicht gespeichert (niedriger als "
                    f"bestehender Score {entry['score']})"
                )
...
class Tetris:
    def __init__(self, width, height, player_name):
        ...
        print(
            f"Aktueller Top-Score: {self.top_score['score']} "
            f"von {self.top_score['name']}"
        )
    ...
    def try_rotation(self):
        ...
        if self.valid_move(self.current_piece, rotation=1):
            ...
            # Für I-Tetromino besondere Behandlung (braucht mehr Platz)
                ...
                self.current_piece.shape == SHAPES[0]
                and self.valid_move(self.current_piece, x_offset=-2, rotation=1)
            ):
    ...
    def clear_lines(self):
        ...
        if self.level > old_level:
            ...
            print(
                f"Level up! Neues Level: {self.level}, "
                f"Geschwindigkeit: {self.current_fall_speed}"
            )
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            pygame.draw.rect(
                screen,
                grid[i][j],
                (PLAY_X + j * GRID_SIZE, PLAY_Y + i * GRID_SIZE,
                 GRID_SIZE, GRID_SIZE),
                0,
            )
    ...
    for i, line in enumerate(form):
        ...
        for j, column in enumerate(row):
            if column == "O":
                ...
                pygame.draw.rect(
                    screen,
                    tuple(min(c + 30, 255) for c in piece.color),
                    highlight_rect,
                    1,
                )
...
def draw_game_over(screen, score, player_name, highscores):
    ...
    screen.blit(
        player_text, (WIDTH // 2 - player_text.get_width() // 2, 180)
    )
    screen.blit(
        score_text, (WIDTH // 2 - score_text.get_width() // 2, 220)
    )
...
def name_input_screen(screen):
    ...
    while running:
        ...
        screen.blit(
            title_shadow,
            (WIDTH // 2 - title_text.get_width() // 2 + shadow_offset, 100 + shadow_offset),
        )
