...
try:
    ...
    # Aktuelle Dimensionen, die im Vollbildmodus angepasst werden
    paddle_width, paddle_height = base_paddle_width, base_paddle_height
    ...
    # Geschwindigkeiten
    paddle_speed = 10
    ai_speed = 6  # Etwas langsamer für einfacheren Modus
    base_ball_speed = 3.5  # Etwas langsamer für einfacheren Modus
    ...
    def reset_game():
        ...
        global paddle1, paddle2, ball, score1, score2, ball_speed_x, ball_speed_y, ai_hit_counter, ai_force_miss
    ...
    def toggle_fullscreen():
        ...
        global screen, fullscreen, width, height, ball_speed_x, ball_speed_y, paddle_width, paddle_height, ball_size, paddle_speed, ai_speed
        fullscreen = not fullscreen
        
        # Merken Sie sich die aktuelle Richtung
        dir_x = 1 if ball_speed_x > 0 else -1
        dir_y = 1 if ball_speed_y > 0 else -1
        ...
        if fullscreen:
            ...
            # Wurzelfaktor für moderatere Skalierung
            scale_factor_x = math.sqrt(scale_x)
            scale_factor_y = math.sqrt(scale_y)
            
            # Spielobjekte und Geschwindigkeiten anpassen
            ...
            # Geschwindigkeit mit neuen Faktoren anwenden
            ball_speed_x = base_ball_speed * scale_factor_x * dir_x
            ball_speed_y = base_ball_speed * scale_factor_y * dir_y
        else:
            width, height = base_width, base_height
            screen = pygame.display.set_mode((width, height))
            ...
            ball_speed_x = base_ball_speed * dir_x
            ball_speed_y = base_ball_speed * dir_y
        
        reset_game()  # Spiel zurücksetzen, damit Positionen angepasst werden
    ...
    def draw_button(text, x, y, w, h, inactive_color, active_color, action=None):
        ...
        button_rects[text] = (button_rect, action)  # Speichert das Button-Rechteck und die Aktion
        ...
        # Button mit passender Farbe zeichnen
    ...
    def handle_button_click(pos):
        ...
        for button_text, (rect, action) in button_rects.items():
    ...
    def menu():
        ...
        # Button-Rechtecke für diesen Bildschirm löschen
        button_rects = {}
        ...
        draw_button("Einzelspieler", button_x, 250, button_width, button_height, GRAY, LIGHT_GRAY, select_singleplayer)
        draw_button("Mehrspieler", button_x, 320, button_width, button_height, GRAY, LIGHT_GRAY, select_multiplayer)
        draw_button("Vollbild", button_x, 390, button_width, button_height, GRAY, LIGHT_GRAY, toggle_fullscreen)
        draw_button("Beenden", button_x, 460, button_width, button_height, GRAY, LIGHT_GRAY, quit_game)
    ...
    def mode_select():
        ...
        # Modus-Anzeige
        mode_text = small_font.render("Einzelspieler" if is_singleplayer else "Mehrspieler", True, WHITE)
        ...
        draw_button("Normal", button_x, 250, button_width, button_height, GRAY, LIGHT_GRAY, start_normal_mode)
        draw_button("Hardcore", button_x, 320, button_width, button_height, GRAY, RED, start_hardcore_mode)
        draw_button("Zurück", button_x, 390, button_width, button_height, GRAY, LIGHT_GRAY, return_to_menu)
    ...
    def game_over():
        ...
        winner_surface = font.render(winner_text, True, RED if is_hardcore else WHITE)
        screen.blit(winner_surface, (width//2 - winner_surface.get_width()//2, height//2 - 50))
        
        score_surface = font.render(f"{score1} - {score2}", True, RED if is_hardcore else WHITE)
        screen.blit(score_surface, (width//2 - score_surface.get_width()//2, height//2 + 30))
        
        # Button zum Zurückkehren zum Hauptmenü
        draw_button("Zurück zum Menü", width//2 - 150, height//2 + 120, 300, 50, GRAY, LIGHT_GRAY, return_to_menu)
        
        pygame.display.flip()
    
    def game_loop():
        """Hauptspielschleife"""
        global score1, score2, ball_speed_x, ball_speed_y, current_state, ai_frame_counter, ai_target_offset
        global ai_hit_counter, ai_hits_until_miss, ai_force_miss
        
        keys = pygame.key.get_pressed()
        
        # Spieler 1 (linker Schläger) - immer W/S
        ...
        if is_singleplayer:
            # Verbesserte KI-Steuerung für Einzelspielermodus
            if ball_speed_x > 0:  # Ball bewegt sich zum rechten Schläger
                ai_frame_counter += 1
                
                # Prüfen, ob KI absichtlich fehlen soll
                if ai_force_miss:
                    # Absichtlich falsch zielen
                    target_y = ball.centery + 200 * (1 if ball.centery < height/2 else -1)
                else:
                    # Gelegentliche Fehler einbauen
                    if random.random() < ai_mistake_chance and ai_frame_counter % 60 == 0:
                        # KI macht einen Fehler und zielt falsch
                        ai_target_offset = random.randint(-50, 50)
                    
                    # Berechneter Zielpunkt mit Ungenauigkeit
                    target_y = ball.centery + ai_target_offset
                    
                    # Begrenzte Vorhersage - KI kann Ball nicht perfekt treffen
                    if random.random() > ai_precision:
                        # Ungenaue Bewegung
                        target_y += random.randint(-30, 30)
                
                # KI bewegt sich zum berechneten Zielpunkt
                if paddle2.centery < target_y and paddle2.bottom < height:
                    paddle2.y += ai_speed
                elif paddle2.centery > target_y and paddle2.top > 0:
                    paddle2.y -= ai_speed
                
                # Zurücksetzen des Zählers für nächste Reaktion
                if ai_frame_counter > ai_reaction_delay:
                    ai_frame_counter = 0
        else:
            # Menschlicher Spieler 2 im Mehrspielermodus
        ...
        if ball.colliderect(paddle1) or ball.colliderect(paddle2):
            if ball.colliderect(paddle2) and ball_speed_x > 0:
                ...
                # Prüfen, ob die KI den nächsten Ball durchlassen soll
                if ai_hit_counter >= ai_hits_until_miss:
                    ai_force_miss = True
                    ai_hit_counter = 0
                    ai_hits_until_miss = random.randint(3, 6)  # Neue zufällige Anzahl festlegen
            
            ball_speed_x *= -1
            
            # Im Hardcore-Modus: Ball wird nach jedem Treffer 10% schneller
            if is_hardcore:
                ball_speed_x *= 1.10
                ball_speed_y *= 1.10
            
            # Etwas Varianz in der Y-Geschwindigkeit
            ball_speed_y += random.uniform(-1, 1)
            # Geschwindigkeit begrenzen
            if abs(ball_speed_y) > 8:
                ball_speed_y = 8 * (1 if ball_speed_y > 0 else -1)
        
        # Ball außerhalb des Spielfelds - auf linker Seite
        if ball.left <= 0:
            score2 += 1
            ...
            # Wurzelfaktor für moderatere Geschwindigkeit
            if fullscreen:
                speed_factor_x = math.sqrt(speed_factor_x)
                speed_factor_y = math.sqrt(speed_factor_y)
        ...
        # Ball außerhalb des Spielfelds - auf rechter Seite
        if ball.right >= width:
            score1 += 1
            ball.x, ball.y = width // 2 - ball_size // 2, height // 2 - ball_size // 2
        ...
        # Zeichnen mit Farben je nach Modus
        screen.fill(BLACK)
        
        # Spielelemente zeichnen (rot im Hardcore-Modus)
    ...
    running = True
    while running:
        # Events zuerst behandeln
        handle_events()
        
        # Aktuellen Spielzustand verarbeiten
